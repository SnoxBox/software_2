...
const modalImage = document.querySelector('body > dialog:nth-child(2) > img:nth-child(2)');
const modal = document.querySelector('dialog');
const modalCloseButton = modal.querySelector('span');

for (let i = 0; i < picArray.length; i++) {
  const newPic = document.createElement("article");
  newPic.setAttribute('class', 'card');

  const newpicH2 = document.createElement('h2');
  newpicH2.textContent = picArray[i].title;

  const newPicFig = document.createElement('figure');
  const newPicFigCpt = document.createElement('figcaption');
  const newPicFigImg = document.createElement('img');
  const p = document.createElement('p');
  newPicFigImg.src = picArray[i].image.medium;
  newPicFigImg.setAttribute('alt', picArray[i].title);
  newPicFigCpt.textContent = picArray[i].caption;
  newPicFig.appendChild(newPicFigImg);
  newPicFig.appendChild(newPicFigCpt);
  p.textContent = picArray[i].description;

  newPic.appendChild(newpicH2);
  newPic.appendChild(newPicFig);
  targetElement.appendChild(newPic);
  targetElement.appendChild(p);

   // add event listener to open modal
   newPic.addEventListener('click', () => {
    modalImage.src = picArray[i].image.large;
    modalImage.setAttribute('alt', picArray[i].title);
    modal.showModal();
  });
}
